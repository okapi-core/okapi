...
public final class TestCorpus {
  ...
  public List<LogIngestRecord> getIndividualRecords() {
    ...
    out.add(
        new LogIngestRecord(base + 1, tA, 20, "GET /api/catalog/search?q=running+shoes user=u123"));
    out.add(
        new LogIngestRecord(
            base + 2, tA, 10, "PricingEngine applied discount code=SPRING10 order=1001"));
    out.add(new LogIngestRecord(base + 3, tA, 30, "Inventory low for sku=SHOE-RED-42 remaining=2"));
    out.add(
        new LogIngestRecord(
            base + 4, tA, 20, "Payment authorized order=1001 amount=79.99 provider=stripe"));
    ...
    out.add(
        new LogIngestRecord(base + 6, tB, 20, "GET /api/catalog/product/SHOE-BLUE-41 user=u999"));
    out.add(
        new LogIngestRecord(
            base + 7,
            tB,
    ...
    out.add(
        new LogIngestRecord(
            base + 9, tB, 40, "Order creation failed order=2002 cause=payment_error"));
  ...
  public void checkQueryProcessorUsualRoute(LogsQueryProcessor qp, String stream, QueryConfig qc)
      throws Exception {
    ...
    var tA =
        qp.getLogs(
            stream, start, end, new LogPageTraceFilter("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"), qc);
