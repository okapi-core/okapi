...
public class TraceQueryServiceTest {
  ...
  void setup() throws Exception {
    dir = Files.createTempDirectory("okapi-trace-query");
  }
  ...
  private static ExportTraceServiceRequest makePayload(
        ...
        Span.newBuilder()
            ...
            .setParentSpanId(
                parentSpanId == null
                    ...
                    : com.google.protobuf.ByteString.copyFrom(hex(parentSpanId)))
    ...
    ResourceSpans rs =
        ResourceSpans.newBuilder().setResource(r.build()).addScopeSpans(scope).build();
  ...
  void getSpans_byTraceId_returnsSorted() throws Exception {
    ...
    page1.append(
        makePayload(
            "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", "1111111111111111", null, base + 10, base + 20));
    page1.append(
        makePayload(
    ...
    page2.append(
        makePayload(
            "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb", "3333333333333333", null, base + 50, base + 60));
    writer.write(tenant, app, page2);

    try (var svc = new TraceQueryService(dir)) {
      List<Span> spans =
          svc.getSpans(base, base + 1000, tenant, app, "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa");
      assertEquals(2, spans.size());
      assertEquals(
          "1111111111111111", HexFormat.of().formatHex(spans.get(0).getSpanId().toByteArray()));
      assertEquals(
          "2222222222222222", HexFormat.of().formatHex(spans.get(1).getSpanId().toByteArray()));
    }
  }
  ...
  void getSpans_byAttributeFilter_exact_and_pattern() throws Exception {
    ...
    var kv2 =
        KeyValue.newBuilder()
            .setKey("service.name")
            .setValue(AnyValue.newBuilder().setStringValue("payments"))
            .build();
  ...
  void getTrace_includes_original_and_parents() throws Exception {
    ...
    try (var svc = new TraceQueryService(dir)) {
      ...
      var ids =
          chain.stream().map(s -> HexFormat.of().formatHex(s.getSpanId().toByteArray())).toList();
