...
public class TraceFileWriterIdleCloseTest {
  ...
  private static ExportTraceServiceRequest payload(long startMs, long endMs) {
    Span sp = Span.newBuilder()
        .setTraceId(com.google.protobuf.ByteString.copyFrom(hex("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")))
  ...
  void idleClose_sweepsOpenStreams() throws Exception {
    ...
    try {
      TraceWriterConfig cfg = TraceWriterConfig.builder().idleCloseMillis(1).reapIntervalMillis(1).build();
