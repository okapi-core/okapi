...
class MemberSetQueryProcessorTest {
  ...
  void returnsEmpty_whenFanOutTrue_orSelfTarget_orOutsideHour() throws Exception {
    ...
    WhoAmI self = new WhoAmI() {
      @Override public String getNodeId() { return "self"; }
      @Override public String getNodeIp() { return "127.0.0.1"; }
      @Override public int getNodePort() { return 8080; }
    };
    ...
    when(selector.select(any(), any(), anyLong(), anyInt(), any())).thenReturn(
        new Member("self", self.getNodeIp(), self.getNodePort()));

    MemberSetQueryProcessor qp =
        new MemberSetQueryProcessor(list, selector, self, new OkHttpClient(), new ObjectMapper());
    ...
    // outside hour
    assertTrue(
        qp.getLogs("t", "s", hourStart - 3600_000L, hourStart - 1, new RegexFilter(".*"),
            QueryConfig.defaultConfig()).isEmpty());
    // fanOut
    assertTrue(
        qp.getLogs("t", "s", hourStart, hourStart + 1000, new RegexFilter(".*"),
            new QueryConfig(true, true, true, true)).isEmpty());
    // self target
    assertTrue(
        qp.getLogs("t", "s", hourStart, hourStart + 1000, new RegexFilter(".*"),
            QueryConfig.defaultConfig()).isEmpty());
  ...
  void callsRemote_andReturnsConvertedItems() throws Exception {
    ...
    var qp = new MemberSetQueryProcessor(list, selector, self, new OkHttpClient(), mapper);

    // Prepare response
    QueryResponse resp = new QueryResponse();
    LogView v1 = new LogView(); v1.tsMillis = 1L; v1.level = 20; v1.body = "a"; v1.traceId = "x";
    LogView v2 = new LogView(); v2.tsMillis = 2L; v2.level = 30; v2.body = "b"; v2.traceId = "y";
