...
public final class DurationUtil {
     ...
     *  Also accepts plain numbers (e.g., "15" or "0.5") meaning seconds (Grafana step). */
    public static long parseToMillis(String dur) {
        ...
        for (int i = 0; i < s.length(); i++) {
            char c = s.charAt(i);
            if (!(Character.isDigit(c) || c == '.')) { numericOrDecimal = false; break; }
        }
        if (numericOrDecimal) {
            // Supports "15", "0.5", "1.25" -> seconds
            java.math.BigDecimal seconds = new java.math.BigDecimal(s);
            return seconds.multiply(java.math.BigDecimal.valueOf(1000))
        ...
        switch (u) {
            ...
            case 'y': return n * 365L * 86_400_000L;
            default:  throw new IllegalArgumentException("Unsupported unit: " + u + " in " + dur);
        }
    }
}
