...
public class LabelMatchTest {
    ...
    void regex_metricName_positive_matchesCpuOnly() {
        ...
        assertTrue(out.stream().allMatch(s -> s.metric().startsWith("cpu_")), "non-cpu metric leaked into result");
        // In our fixture, cpu_usage has two instances (i1, i2)
        var instances = out.stream().map(s -> s.labels().tags().get("instance")).collect(java.util.stream.Collectors.toSet());
    ...
    void regex_label_negative_excludes_i2_instance() {
        ...
        assertTrue(out.stream().noneMatch(s -> "i2".equals(s.labels().tags().get("instance"))),
