...
public final class MockTimeSeriesClient implements TsClient {
    ...
    public void putHisto(String metric, Map<String,String> tags, long startMs, long endMs,
                         List<Float> ubs, List<Integer> counts) {
        var key = new Key(metric, tags);
        histoStore.put(key, new HistoScan("", startMs, endMs, List.copyOf(ubs), List.copyOf(counts)));
    }

    @Override
    public Scan get(String name, Map<String, String> tags,
        ...
        if (series == null) {
            return GaugeScan.builder().universalPath("")
                .timestamps(List.of())
                .values(List.of())
                .build();
        }
